...
A Dockerized Node service that exposes the target repo via Telegram. Send text or voice; the bot pulls the repo, spawns Claude Code, replies, commits & pushes any changes.
...
Source layout, [design doc](../../docs/process/specs/2026-05-17-clawde-design.md), and the [implementation plan](../../docs/process/plans/2026-05-17-clawde.md) live alongside.
...
- An SSH key that has **write** access to the target repo (deploy key works fine)
...
```bash
ssh-keygen -t ed25519 -C "clawde@$(hostname)" -f ssh-deploy-key -N ""
...
`cat ssh-deploy-key.pub` → copy → https://github.com/<owner>/<repo>/settings/keys/new → paste, title `clawde`, ✅ **Allow write access**, **Add key**.
...
Produces `clawde:latest`. ~734 MB; includes Node 24, Claude Code CLI, git, openssh.
...
```bash
docker compose run --rm -it --entrypoint="" clawde claude
```
...
cloning_repo     repoUrl=...        ← first boot only
...
Expect: *"clawde ready. Send text or voice. /reset clears the session."*
...
The bot's `git clone` uses the target repo's **default branch** (usually `master` / `main`) and `git pull --rebase` against the upstream of whatever branch is checked out. If you develop new bot features on a branch first:

```bash
# tell the in-container repo to track your branch
docker exec clawde sh -c "cd /data/repo && git fetch && git checkout feature/your-branch"
```
...
docker compose down -v                # stop, drop ALL volumes (repo, db, home)
docker exec -it clawde bash  # shell into the running container
...
|---|---|---|
| `telegram-bot_repo-data` | `/data/repo` | Cloned target repo |
...
```bash
docker exec clawde curl -s -X POST http://127.0.0.1:8080/notify \
...
The in-container target repo may be on a different branch than the file you expect. Check:

```bash
docker exec clawde sh -c "cd /data/repo && git branch --show-current && git log -1 --oneline"
```
...
The bot is the sole consumer of Telegram updates. For each user message it acquires a file-lock on the target repo's `.git/`, pulls, spawns `claude -p --resume <sid> --append-system-prompt prompts/telegram-mode.md --dangerously-skip-permissions` from `/data/repo`, parses stdout for an optional trailing ` ```tg ` JSON block, sends the body (with inline keyboard if present), then commits + pushes any working-tree changes. Sessions are stored in SQLite (per-Telegram-user → Claude session id, with a configurable TTL). Proactive notifications from Claude during long tasks go through a localhost Hono `/notify` endpoint that the bot exposes on `127.0.0.1:8080` inside the container, invoked from `notify-tg.sh` (in the target repo, so Claude's Bash tool can reach it).
...
Local execution without Docker is awkward — the entrypoint hard-codes `/data/repo` and `/data/db`. For iterative development, edit source, run `docker compose up -d --build`, then `docker compose logs -f`.
...
Notes:
- Namespace: `app-clawde`.
- Storage class: `local-path`. Three PVCs (`-repo-data`, `-bot-db`, `-bot-home`).
...
Follow logs
```bash
kubectl logs -f deployment/clawde -n app-clawde
```
