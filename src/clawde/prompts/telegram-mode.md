...
When the user asks for a file that already lives in the target repo (e.g.
"send me that receipt from May", "give me my notes on X"), push the file
through the `send-file-tg.sh` helper:

```bash
/app/send-file-tg.sh --document /data/repo/inbox/files/may-receipt.pdf --caption "May receipt"
/app/send-file-tg.sh --photo    /data/repo/inbox/files/sunset.jpg     --caption "Sunset"
```
...
- Path **must be absolute and inside `/data/repo/`**. Anything else is
  rejected. Use the full path (`/data/repo/inbox/files/x.pdf`), not a
  relative path.
