...
function makeDeps(overrides: Partial<{
  ...
  return {
    repoRoot: overrides.repoRoot ?? "/data/repo",
...
describe("send-file app", () => {
  ...
  beforeEach(() => {
    repoRoot = mkdtempSync(join(tmpdir(), "clawde-"));
  });
  afterEach(() => {
    rmSync(repoRoot, { recursive: true, force: true });
  });

  it("sends a document for a valid in-repo path", async () => {
    const filePath = join(repoRoot, "inbox", "files", "report.pdf");
    mkdirSync(join(repoRoot, "inbox", "files"), { recursive: true });
    writeFileSync(filePath, Buffer.from("PDFDATA"));

    const deps = makeDeps({ repoRoot });
  ...
  it("sends a photo for a valid in-repo path", async () => {
    const filePath = join(repoRoot, "sunset.jpg");
  ...
  it("rejects path outside repoRoot with 403", async () => {
  ...
  it("rejects a symlink that escapes repoRoot with 403", async () => {
    const outside = mkdtempSync(join(tmpdir(), "outside-"));
  ...
  it("rejects a sibling directory that shares repoRoot's prefix with 403", async () => {
    // repoRoot looks like "/tmp/clawde-AAA"; create a peer "/tmp/clawde-AAA-evil/secret.txt".
    // The naive prefix check `resolved.startsWith(repoRoot)` would falsely admit this;
  ...
  it("rejects document larger than 50 MB with 413", async () => {
    const filePath = join(repoRoot, "big.bin");
  ...
  it("rejects photo larger than 10 MB with 413", async () => {
    const filePath = join(repoRoot, "big.jpg");
    const { openSync, ftruncateSync, closeSync } = await import("node:fs");
    const fd = openSync(filePath, "w");
  ...
  it("rejects caption longer than 1024 chars with 400", async () => {
    const filePath = join(repoRoot, "x.pdf");
  ...
  it("converts default-mode caption to MarkdownV2 via telegramify-markdown", async () => {
    const filePath = join(repoRoot, "y.pdf");
  ...
  it("passes HTML caption through unchanged with parse_mode HTML", async () => {
    const filePath = join(repoRoot, "z.pdf");
  ...
  it("returns 502 if the Telegram API call throws", async () => {
    const filePath = join(repoRoot, "fail.pdf");
    writeFileSync(filePath, "x");
