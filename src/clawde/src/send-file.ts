...
/**
 * Validate that `rawPath` resolves to a regular file inside the target repo.
 * @param resolvedRepoRoot must already be realpath'd by the caller (no symlinks, no trailing slash)
 */
function resolveRepoPath(rawPath: string, resolvedRepoRoot: string): ResolveResult {
  let resolved: string;
  try {
  ...
  // realpathSync strips trailing slashes, so resolvedRepoRoot never ends with "/"
  const prefix = resolvedRepoRoot + "/";
...
export function createSendFileApp(deps: SendFileDeps) {
  // Resolve once at construction: repoRoot may contain symlinks (e.g. /var → /private/var on darwin).
  const resolvedRepoRoot = realpathSync(deps.repoRoot);
  const app = new Hono();
  app.post("/send-file", async (c) => {
    ...
    const r = resolveRepoPath(path, resolvedRepoRoot);
    if (!r.ok) {
