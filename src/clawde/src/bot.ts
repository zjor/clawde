...
export function createBot(deps: BotDeps): Bot {
  const { config, sessions, git, repoCwd, promptPath } = deps;
  ...
  bot.command("start", (ctx) =>
    ctx.reply("clawde ready. Send text or voice. /reset clears the session.")
  );
  ...
  async function saveUpload(
    ...
    const absDir = join(repoCwd, "inbox", "files");
